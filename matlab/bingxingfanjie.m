...
pitch_range = linspace(0, 60, 600);  % 在0到60度之间分成100份
yaw_range = linspace(45, 135, 600);  % 在45到135度之间分成100份
...
parfor i = 1:length(pitch_range)
    ...
    % 更新进度计数器
    send(progress, (i - 1) * length(yaw_range) + j);
end
...
% 总数据量
total_data_count = length(pitch_range) * length(yaw_range);

% 绘制直方图
figure;
h = histogram(errors(errors < inf), 'Normalization', 'probability');
xlabel('Error');
ylabel('Probability');
title(sprintf('Error Distribution (Total Data Count: %d)', total_data_count));
min_error = min(errors(:));
text(h.BinEdges(2), max(h.Values)*0.9, sprintf('Min Error: %.6f', min_error), 'FontSize', 12, 'Color', 'red', 'FontWeight', 'bold', 'HorizontalAlignment', 'left');

% 绘制三维误差分布图
[pitch_grid, yaw_grid] = meshgrid(pitch_range, yaw_range);
figure;
surf(pitch_grid, yaw_grid, errors');
xlabel('Pitch Angle (degrees)');
ylabel('Yaw Angle (degrees)');
zlabel('Error');
title(sprintf('Error Distribution for Different Pitch and Yaw Angles (Total Data Count: %d)', total_data_count));
colorbar;
grid on;

% 标注最小误差点
hold on;
plot3(pitch_range(min_i), yaw_range(min_j), min_error, 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r');
text(pitch_range(min_i), yaw_range(min_j), min_error, sprintf('Min Error: %.6f', min_error), 'FontSize', 12, 'Color', 'red', 'FontWeight', 'bold', 'HorizontalAlignment', 'left');
