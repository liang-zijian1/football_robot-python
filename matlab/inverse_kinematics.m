function inverse_kinematics(target_position, initial_speed, spin_speed)
    % 设定初始条件范围和优化参数
    pitch_range = [0, pi/2]; % 俯仰角范围
    yaw_range = [0, 2*pi];  % 偏航角范围

    % 优化目标函数
    objective_function = @(angles) compute_trajectory_error(angles, target_position, initial_speed, spin_speed);

    % 粒子群优化选项
    options = optimoptions('particleswarm', 'Display', 'iter', 'SwarmSize', 50, 'HybridFcn', @fmincon);
...
function error = compute_trajectory_error(angles, target_position, initial_speed, spin_speed)
    % 提取角度
    pitch = angles(1);
    yaw = angles(2);
    ...
    % 初始状态向量
    initial_state = [0; 0; 0; v0x; v0y; v0z; spin_speed; 0; 0];
