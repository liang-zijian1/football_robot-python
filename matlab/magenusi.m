function magenusi
    % 初始条件
    initial_conditions = struct('v0', 50, 'pitch',13.0217, 'yaw',  85.4174, 'roll', 180, 'w', 5);
    ...
    function simulate_trajectory(initial_conditions)
        ...
        % 绘制出射点
        scatter3(initial_state(1,1), initial_state(2,1), initial_state(3,1), 100, 'filled', 'k');
     ...
     %   quiver3(0, 0, 0, v0x, v0y, v0z, 'r', 'LineWidth', 2,'AutoScale','on');
      ...
      %  quiver3(0, 0, 0, omega(1), omega(2), omega(3), 'g', 'LineWidth', 2,'AutoScale','on');
        ...
        if isempty(crossing_index)
            ...
            % 标记球打在球门平面上的位置
            plot3(x_hit, goal_y, z_hit, 'ro', 'MarkerSize', 5, 'MarkerFaceColor', 'r');
        end
        ...
        xlim([-penalty_area_width/2-5, penalty_area_width/2+5]);
        ylim([-10, 1.5*goal_y]);
        zlim([0, goal_height * 5]);
