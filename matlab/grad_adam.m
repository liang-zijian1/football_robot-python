...
% 初始化pitch和yaw
pitch = rand() * 60; % 初始pitch角度，范围0-60度
yaw = rand() * 90 + 45; % 初始yaw角度，范围45-135度
...
for iter = 1:num_iterations
    ...
    % 更新Adam优化变量
    m_pitch = beta1 * m_pitch + (1 - beta1) * grad_pitch;
    v_pitch = beta2 * v_pitch + (1 - beta2) * grad_pitch^2;
    m_pitch_hat = m_pitch / (1 - beta1^(iter + 1));
    v_pitch_hat = v_pitch / (1 - beta2^(iter + 1));
    
    m_yaw = beta1 * m_yaw + (1 - beta1) * grad_yaw;
    v_yaw = beta2 * v_yaw + (1 - beta2) * grad_yaw^2;
    m_yaw_hat = m_yaw / (1 - beta1^(iter + 1));
    v_yaw_hat = v_yaw / (1 - beta2^(iter + 1));
    
    % 更新pitch和yaw
    pitch = pitch - learning_rate * m_pitch_hat / (sqrt(v_pitch_hat) + epsilon);
    yaw = yaw - learning_rate * m_yaw_hat / (sqrt(v_yaw_hat) + epsilon);
end
...
% 绘制误差值收敛图
figure;
plot(1:num_iterations, errors, 'b-', 'LineWidth', 2);
...
function grad_pitch = compute_gradient_pitch(pitch, yaw, target_position, initial_speed, spin_speed, roll_angle)
    epsilon = 1e-5;
    hit_position1 = compute_trajectory_error_with_position([pitch + epsilon, yaw], target_position, initial_speed, spin_speed, roll_angle);
    hit_position2 = compute_trajectory_error_with_position([pitch - epsilon, yaw], target_position, initial_speed, spin_speed, roll_angle);
    error1 = norm(hit_position1 - target_position);
    error2 = norm(hit_position2 - target_position);
...
function grad_yaw = compute_gradient_yaw(pitch, yaw, target_position, initial_speed, spin_speed, roll_angle)
    epsilon = 1e-5;
    hit_position1 = compute_trajectory_error_with_position([pitch, yaw + epsilon], target_position, initial_speed, spin_speed, roll_angle);
    hit_position2 = compute_trajectory_error_with_position([pitch, yaw - epsilon], target_position, initial_speed, spin_speed, roll_angle);
    error1 = norm(hit_position1 - target_position);
    error2 = norm(hit_position2 - target_position);
...
function dstate = projectile(~, state, omega)
    ...
    dstate(7) = 0;         % omega_x恒定
    dstate(8) = 0;         % omega_y恒定
    dstate(9) = 0;         % omega_z恒定
end
