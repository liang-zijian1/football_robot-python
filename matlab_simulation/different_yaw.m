function different_yaw
    % 初始条件
    initial_conditions = struct('v0', 22.2, 'pitch', 15, 'yaw', 0, 'roll', 180, 'w', 10);

    % 创建图形窗口和初始轨迹
    fig = figure('Name', '足球轨迹模拟', 'NumberTitle', 'off');
    simulate_trajectory(initial_conditions);
    ...
    uit = uitable(fig, 'Data', struct2cell(initial_conditions), ...
        ...
        'ColumnEditable', true, ...
        'CellEditCallback', @(src, event) update_initial_conditions(src, event, initial_conditions));
    
    function update_initial_conditions(~, event, initial_conditions)
        field = fieldnames(initial_conditions);
        initial_conditions.(field{event.Indices(1)}) = event.NewData;
        simulate_trajectory(initial_conditions);
    end

    function simulate_trajectory(initial_conditions)
        % 清除旧图像
        clf;
        ...
        yaw_angles = [0, 30, 60, 90];  % 不同的 yaw 角度
        
        % 遍历不同的 yaw 角度，绘制轨迹
        for i = 1:length(yaw_angles)
            yaw = deg2rad(yaw_angles(i));
            
            % 计算初始速度的分量
            v0x = v0 * cos(pitch) * cos(yaw);
            v0y = v0 * cos(pitch) * sin(yaw);
            v0z = v0 * sin(pitch);
            ...
            % 绘制运动轨迹
            plot3(x, y, z, 'LineWidth', 2, 'DisplayName', sprintf('Yaw = %d°', yaw_angles(i)));
        end
