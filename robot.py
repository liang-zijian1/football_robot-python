...
class MyRobot(wpilib.TimedRobot):
    ...
    def teleopInit(self) -> None:
        ...
        BL_cancoder_offset=0
        BR_cancoder_offset=-0.437012
       
        #获取电机当前位置
        left_front_steer_motor_postion=self.FL_steer_motor.get_position().value_as_double
        right_front_steer_motor_postion=self.FR_steer_motor.get_position().value_as_double
        right_back_steer_motor_postion=self.BR_steer_motor.get_position().value_as_double
       
        # 计算实际角度与0位置的误差
        FL_cancoder_real_angle=self.FL_CANcoder.get_absolute_position().value_as_double
        left_front_error=-FL_cancoder_real_angle+FL_cancoder_offset
        FR_cancoder_real_angle=self.FR_CANcoder.get_absolute_position().value_as_double
        right_front_error=-FR_cancoder_real_angle+FR_cancoder_offset
        BR_cancoder_real_angle=self.BR_CANcoder.get_absolute_position().value_as_double
        right_back_error=-BR_cancoder_real_angle+BR_cancoder_offset
        
        #设置舵向回中
        self.FL_steer_motor.set_control(PositionDutyCycle(left_front_error*12.84375+left_front_steer_motor_postion))
        self.FR_steer_motor.set_control(PositionDutyCycle(right_front_error*12.84375+right_front_steer_motor_postion))
        self.BR_steer_motor.set_control(PositionDutyCycle(right_back_error*12.84375+right_back_steer_motor_postion))
        
        self.wheel_init_offset_FL=self.FL_steer_motor.get_position().value_as_double
        self.wheel_init_offset_FR=self.FR_steer_motor.get_position().value_as_double
        self.wheel_init_offset_BR=self.BR_steer_motor.get_position().value_as_double
        self.wheel_init_offset_BL=self.BL_steer_motor.get_position().value_as_double
    ...
    def teleopPeriodic(self):
        ...
        #print(wheel_speeds)
        # 将目标角度发送给转向电机
        self.FL_steer_motor.set_control(PositionDutyCycle(self.wheel_init_offset_FL+wheel_speeds[0]['yaw']/3.1415926*12.84375))
