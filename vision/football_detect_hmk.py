...
# 打开视频文件
video_path = 'E:/Desktop/football_robot/code/vision/input_video/90_18.4_0.4_0.1.mp4'#这里要改成实际的视频名称和路径
...
# 创建一个txt文件保存足球坐标及时间
output_txt_name = f'coordinates_{input_video_name}.txt'
with open(output_txt_name, 'w') as f:
    f.write("Frame_Number, Time(s), Center_X, Center_Y\n")
    ...
    for (center_x, center_y), life in updated_detections_history.items():
        if life >= max_life:
            ...
            with open(output_txt_name, 'a') as f:
                f.write(f"{frame_number}, {frame_time:.2f}, {center_x}, {center_y}\n")
